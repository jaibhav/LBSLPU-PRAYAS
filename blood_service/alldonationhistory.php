<?php
    ...
    $sql_users="SELECT users.*,donated_at FROM users
    JOIN donations_list ON users.regno = donations_list.regno";
...
<head>
        <meta charset="utf-8">
        <title>All Donation History</title>
                    ...
                    <tbody>
                        ...
                        <?php
                            while($rows=mysqli_fetch_array($user_query))
                            {
                                ...
                                echo '<td>'.$rows['bgrp'].'</td>';
                                echo '<td>'.$rows['donated_at'].'</td>';
